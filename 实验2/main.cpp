...
int main(int argc, char *argv[])
{
	...
	for (j = 0; j<2000; j++)
	{
		...
		if (my_rank != 0)
		{
			MPI_Bsend(&(cars[i].poisition), 1, MPI_INT, my_rank - 1, j * 10 + my_rank, MPI_COMM_WORLD);
		}
	...
	if (my_rank == 0)MPI_Send((cars), sizeof(car)*num_car / 4, MPI_BYTE, 3, my_rank, MPI_COMM_WORLD);
	if (my_rank == 1)MPI_Send((cars + num_car / 4), sizeof(car)*num_car / 4, MPI_BYTE, 3, my_rank, MPI_COMM_WORLD);
	if (my_rank == 2)MPI_Send((cars + 2 * num_car / 4), sizeof(car)*num_car / 4, MPI_BYTE, 3, my_rank, MPI_COMM_WORLD);
	if (my_rank == numprocs - 1){
		MPI_Status status;
		MPI_Recv((cars), sizeof(car)*num_car / 4, MPI_BYTE, 0, 0, MPI_COMM_WORLD, &status);
		MPI_Recv((cars + num_car / 4), sizeof(car)*num_car / 4, MPI_BYTE, 1, 1, MPI_COMM_WORLD, &status);
		MPI_Recv((cars + 2 * num_car / 4), sizeof(car)*num_car / 4, MPI_BYTE, 2, 2, MPI_COMM_WORLD, &status);
